...
Transmission.prototype.removeBefore = function(config, callback) {
  if (config === null) {
    return callback(new Error('Configuration object is invalid'));
  }

  var threshold = moment().subtract(config.amount, config.unit);
  if (!threshold.isValid()) {
    return callback(new Error('Threshold is not a valid moment.js object'));
  }

  config.keepFiles = config.keepFiles || true;
  config.unit = config.unit || 'days';
  ...
  this.get(function(err, result) {
    ...
    result.torrents.forEach(function(torrent) {
      var torrentCreated = moment.unix(torrent.addedDate);
      if (!torrentCreated.isValid()) {
        return callback(new Error('Torrent ' + torrent.id + ' does not have a valid addedDate property'));
      }
      if (torrentCreated.isBefore(threshold)) {
        this.remove(torrent.id, config.keepFiles, function() {
